...
plugins-install-{{ plugin }}:
  cmd.run:
    - name: {{ warp10.warpfleet.binary }} get {{ default_options }} {{ warp10.warpfleet.plugins[plugin].repository }} {{ plugin }} {{ warp10.warpfleet.plugins[plugin].version }}
    - creates:
      - {{ warp10.path }}/lib/{{ warp10.warpfleet.plugins[plugin].file_to_check }}
{% endfor %}
